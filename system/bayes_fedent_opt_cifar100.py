...
CONFIG_DIR = REPO / "auto_configs_cifar100"; CONFIG_DIR.mkdir(exist_ok=True)
LOG_DIR    = REPO / "logs_cifar100"; LOG_DIR.mkdir(exist_ok=True)
HIST_CSV   = REPO / "history_cifar100.csv"
MAIN_PY    = SYS / "main.py"
...
def objective(trial, args):
    ...
    steps = {
        'coarse': {
            'stage1': {'gr': 40, 'H_min': 0.5, 'alpha_vp': 0.5, 'beta': 0.5, 'buffer_size': 10},
            'stage2': {'gr': 40, 'H_min': 0.5, 'alpha_vp': 0.5, 'beta': 0.5, 'buffer_size': 10},
            'stage3': {'gr': 20, 'H_min': 0.5, 'alpha_vp': 0.5, 'beta': 0.5, 'buffer_size': 10},
        },
        'fine': {
    ...
    # coarse
    ranges = {
    'stage1': {'gr': (40, 200), 'H_min': (0.0, 3.5), 'alpha_vp': (0.0, 3.0), 'beta': (0.0, 3.0), 'buffer_size': (0, 60)},
    'stage2': {'gr': (240, 400), 'H_min': (0.0, 3.5), 'alpha_vp': (0.0, 3.0), 'beta': (0.0, 3.0), 'buffer_size': (0, 60)},
    'stage3': {'gr': (500, 500), 'H_min': (0.0, 3.5), 'alpha_vp': (0.0, 3.0), 'beta': (0.0, 3.0), 'buffer_size': (0, 60)},
    }
    ...
    cmd = (f"cd {SYS} && python {MAIN_PY.name} --dataset {args.dataset} --num_classes 20 --model CNN "
...
if __name__ == "__main__":
    ...
    ap.add_argument("--study",    default="fedent_opt_cifar100.db")
    ...
    study = optuna.create_study(direction="maximize",
                                study_name="FedEntSmooth_BO_cifar100",
                                storage=storage,
                                load_if_exists=True)
